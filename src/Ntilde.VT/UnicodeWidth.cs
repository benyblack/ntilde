...
// Partial: the East_Asian_Width table lives in UnicodeWidth.EastAsianWide.g.cs, generated by
// scripts/gen-unicode-width.py.
public static partial class UnicodeWidth
{
    ...
    // Binary search over the generated (start, end) pairs. The bounds check runs first so that
    // everything below U+1100 (ASCII, Latin, Greek, Cyrillic, ...), which is nearly all terminal
    // output, returns without searching.
    internal static bool IsEastAsianWide(int codePoint)
    {
        if (codePoint < EastAsianWideMin || codePoint > EastAsianWideMax)
        {
            return false;
        }

        ReadOnlySpan<int> ranges = EastAsianWideRanges;
        int lo = 0;
    ...
    // Bases whose width depends on presentation: one cell as text, two when VS16 asks for emoji.
    // The SMP symbol blocks belong here as well as Misc Symbols/Dingbats. Their text-default
    // pictographs (U+1F321 THERMOMETER, U+1F3F3 WHITE FLAG, U+1F441 EYE, ...) are EAW=N and so
    // narrow as bare runes, but they are usually sent with VS16, and the fully-qualified form must
    // stay two cells. The EAW=W emoji in those blocks are two cells whatever the presentation, so
    // including them changes nothing.
    private static bool IsAmbiguousEmojiBase(Rune rune)
        => (rune.Value >= 0x2600 && rune.Value <= 0x27BF) || IsEmojiRange(rune);

    // The SMP symbol and pictograph blocks. A block span, not a width claim: GetRuneWidth decides
    // width.
